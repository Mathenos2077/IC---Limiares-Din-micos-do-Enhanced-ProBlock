...
def inserir_avaliacao(conn, numero, cdf, ic, co_score_total, subject_score_total):
    conn.execute("""
        INSERT INTO AVALIACAO (numero, CDF, IC, coScoreTotal, subjectScoreTotal)
        VALUES (?, ?, ?, ?, ?)
    """, (numero, cdf, ic, co_score_total, subject_score_total))
...
def inserir_relacionamento_interpreta(conn, avaliacao_numero, modelo_nome, conclusao, limiar_score):
    conn.execute("""
        INSERT INTO INTERPRETA (avaliacao_numero, modelo_nome, conclusao, limiarScore)
        VALUES (?, ?, ?, ?)
    """, (avaliacao_numero, modelo_nome, conclusao, limiar_score))